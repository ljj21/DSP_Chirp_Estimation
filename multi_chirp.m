function [start_time, init_freq, duration, bandwidth, detected_alpha] = multi_chirp(observed_signal, signal_num, fs, fLevel)
    ...
    [~, ~, ~, ~, alpha2] = extract_chirp_component(scaled_data, fs, fLevel, 5 * signal_num);
    ...
    for k = 1:length(alpha2)
        ...
        for index = 1:valid_cnt
            ...
            if (duration_ < 2e-6 ...
                    || (abs(init_freq(index) - init_freq_) / abs(init_freq(index)) < 0.02 ...
                    && abs(duration(index) - duration_) / abs(duration(index)) < 0.02 ...
                    && abs(detected_alpha(index) - alpha) / abs(alpha) < 0.02))
                isvalid = 0;
                break;
            end
